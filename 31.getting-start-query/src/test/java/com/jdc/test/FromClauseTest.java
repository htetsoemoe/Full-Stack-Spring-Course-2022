...
public class FromClauseTest extends AbstractTest{
	...
	void test_name_and_phone(String name, String phone, int count) {
		var jpql = "select c from Customer c where name like :name and phone like :phone";// JPQL SELECT Statement and WHERE Statement
	...
	void test_member_of(String color, long count) {
		var jpql = "select count(p) from Product p where :color member of p.colors"; // JPQL MEMBER OF Statement which use in collection type 
	...
	void test_between(LocalDate from, LocalDate to, int count) {
