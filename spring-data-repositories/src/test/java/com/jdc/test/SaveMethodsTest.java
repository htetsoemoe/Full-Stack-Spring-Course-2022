...
public class SaveMethodsTest {
	...
	void test_save(String name, Type type, String region, String capital, int porpolution) {
		
		// Prepare Inputs
		var input = new State(name, type, region, capital, porpolution);
		
		// Execute Test Method
		var result = repo.save(input); // save can use for 'insert' and 'update'
	...
	void test_update(int id, String name, Type type, String region, String capital, int porpulation) {
		...
		var result = repo.findById(id).get();// If a value is present, returns the value, otherwise throws NoSuchElementException.
